...
def train_sme_model():
    print("🚀 Iniciando proceso de entrenamiento: Regresión Logística (SMEs) con Optimización...")
    ...
    print(f"✨ Mejor C: {grid_search.best_params_['C']:.5f} (AUC CV: {grid_search.best_score_:.4f})")
    ...
    plt.plot(fpr_train, tpr_train, color=TRAIN_COLOR, lw=2.5, 
             label=f'Train ROC (AUC = {auc_train:.4f})', alpha=0.9)
    plt.plot(fpr_test, tpr_test, color=TEST_COLOR, lw=4, 
    ...
    plt.title('Performance Analysis: ROC Curves', color='white', fontsize=18, fontweight='bold', pad=30)
    ...
    plt.savefig('model_assets/sme_logistic_roc.png', bbox_inches='tight', facecolor=BKG_COLOR)
    plt.close()
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap=sns.color_palette("ch:start=.2,rot=-.3", as_cmap=True), 
                cbar=False, annot_kws={"size": 16, "weight": "bold", "color": "white"})
    plt.title('Confusion Matrix: Predictive Accuracy', fontsize=16, fontweight='bold', pad=25, color='white')
    ...
    plt.savefig('model_assets/sme_logistic_cm.png', bbox_inches='tight', facecolor=BKG_COLOR)
    plt.close()

    # 9. Guardar Artefactos
    with open('model_assets/sme_logistic_v1.pkl', 'wb') as f: pickle.dump(model, f)
    with open('model_assets/sme_scaler_v1.pkl', 'wb') as f: pickle.dump(scaler, f)
    with open('model_assets/feature_names.txt', 'w') as f:
        for col in X_encoded.columns: f.write(f"{col}\n")
